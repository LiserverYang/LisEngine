...
def BuildModule(ModuleName: str):
    ...
    if BuildContext.TargetName != "Reflection" and BuildContext.ModuleConfiguration[ModuleID].EnableReflectionGeneric:
        SearchReflection(FileIO(os.path.dirname(
            BuildContext.ModulePath[ModuleID]) + "/Public/"))
        for File in ReflectionHppFileList:
            GenericReflectionFiles(FileIO(File), FileIO(
                "./Build/Binaries/Reflection"))
            GenericFilePath: str = os.path.dirname(
                File) + f"/{os.path.splitext(File)[0].split('/')[-1]}.gen.cpp"
    ...
    if BuildContext.ModuleConfiguration[ModuleID].BinaryType == BinaryTypeEnum.EntryPoint:
        TargetExits = FileIO(
            f"./Build/Binaries/{ModuleName}{PlatFormEndSwitchExe}").Exits()
    elif BuildContext.ModuleConfiguration[ModuleID].BinaryType == BinaryTypeEnum.DynamicLib:
        TargetExits = FileIO(
            f"./Build/Binaries/{BuildContext.TargetName}-{ModuleName}{PlatFormEndSwitchDy}").Exits()
    else:
        TargetExits = FileIO(
            f"./Build/Binaries/{BuildContext.TargetName}-{ModuleName}.a").Exits()
